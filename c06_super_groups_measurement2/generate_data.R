...
get_group_data <- function(supergroup) {
  ...
  data.frame(Observation = obs_vals, 
             Variable = var_vals, 
             Measurement = runif(total_vals, 1 + meas_min_val, 1 + meas_max_val),
             Measurement2 = meas_vals,
             Group = group_vals,
             Supergroup = paste0("supergroup", supergroup))
}

data <- do.call(rbind, lapply(seq(num_super_groups), FUN = function(x) {get_group_data(x)}))
write.table(data, "c06_super_groups_measurement2/c06_super_groups_measurement2.tsv", sep = "\t", row.names = FALSE)
